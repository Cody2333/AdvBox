...
class PytorchModel(Model):
    ...
    def gradient(self, data, label):
        ...
        min = np.min(cov_abs)
        append_matrix = np.eye(28*28) * min * 0.0000001

        covI = np.linalg.inv(cov + append_matrix)
        u = np.load('../u.data.npy')
        covI = torch.from_numpy(covI).to(self._device)
        u = torch.from_numpy(u).to(self._device)
        scaled_data = self._process_input(data)
        ...
        tt=scaled_data.reshape(28*28).float()
        mat = (tt-u).reshape(28*28, 1)
        ...
        # print(sum1,sum2)
        # print(grad1, grad2)
        grad = grad1 * 1 + sum1 /sum2 * grad2 * 0.001 # 负数为拉近距离，正数为拉远距离
        # print(torch.mean(grad2) * sum1/sum2)
        # print(grad1)
        # print(grad2 * sum1 /sum2)
        # print(grad)
        print(loss)
